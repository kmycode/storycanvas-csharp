...
{
	public partial class StoryDatePicker : ContentPage
	{
		private bool IsEnabledValue;
		private DatePickerMessage Message;
		private bool isCleared = false;

		public StoryDatePicker(DatePickerMessage message)
		{
			InitializeComponent();
			...
			this.Disappearing += (sender, e) =>
			{
				if (!this.isCleared)
				{
					...
					{
						this.Message.Action.Invoke(this.Message.Calendar.Date(this.GetYear(), this.GetMonth(), this.GetDay()));
					}
				}
			};
			...
			{
				this.YearInput.Text = message.DefaultValue.Year.ToString();
				this.MonthInput.SelectedIndex = message.DefaultValue.Month - 1;
				this.DayInput.SelectedIndex = message.DefaultValue.Day - 1;
			}
